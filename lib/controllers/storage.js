...
class Storage extends Controller {
  ...
  async get (head = false) {
    ...
    if (await this.checkToken('r')) {
      ...
      const status = !item || (item.items && !Object.keys(item.items).length) ? 404 : 200;
      ...
      if (item['Content-Length'] && head) this._headers['Content-Length'] = item['Content-Length'];
  ...
  async checkToken (permission) {
    ...
    try {
      if (!this._token) {
        this.unauthorized(401, 'missing_token');
        return false;
      }
